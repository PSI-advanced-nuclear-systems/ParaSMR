...
module docstring of homogenise.py for the conservation identity.
...
────────
  ...
  2. per component, by type:
       cylinder (default)      homogenise() -> equivalent cylinder, placed at
...
def homogenise_assembly(
    ...
    for spec in cylinder_specs:
        ...
        if key not in cache:
            ...
            cache[key] = homogenise(spec, materials)
        else:
...
def build_cad(
    ...
    ``units`` is required only when ``export_path`` is given, and means exactly
    what it means for assemble_objects() — the unit the model's numbers are in,
    written into the STEP header. The homogenisation itself never needed it.
    """
    import cadquery as cq
    ...
    if export_path is not None:
        if units is None:
            raise ValueError(
                "build_cad: 'units' is required when exporting — the STEP format "
                "demands a length-unit declaration. The homogenised model itself "
                "is unit-agnostic; this only says what its numbers mean."
            )
