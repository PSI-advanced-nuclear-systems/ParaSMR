...
TOP_PLATE = {
    ...
    # Holes for IHX, pumps, and above_core_structure are auto-generated
...
def _make_ihx(obj_id, angle_deg):
    return {
        ...
        "bundle_shell_window_z_from_top": 1.5, #1, #0.33,  # gap from z_up_bot to window top (default 0.3 + 10%)
...
# Half-section A → B → C revolved 360° about Z. The A–B–C polyline is the
# OUTER surface of the shell (the wall grows inward by "thickness"), so:
...
ABOVE_CORE_STRUCTURE = {
    ...
    "top_cyl_outer_r":      1.750, #1.843, #1.200,           # shrunk to clear pumps & IHX
    ...
    # z_bottom is auto-computed by resolver: top cylinder bottom aligns with top plate top
    "crdl": {
        "through_d":          0.080,
        "pitch":              0.300,
        "pipe_wall_t":        0.005,
